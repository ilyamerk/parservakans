...
from build_job_analytics import (
    write_excel,
    sanitize_sheet_name,
    unique_sheet_name,
    RATES_SHEET_BASE,
...
class HourlyRateComputationTests(unittest.TestCase):
    ...
    def test_compute_hourly_rate_infers_hours_from_schedule(self):
        vac = {
            "ЗП от (т.р.)": 150,
            "График": "24/48",
        }
    ...
    def test_compute_hourly_rate_detects_hours_from_text(self):
        vac = {
            "ЗП от (т.р.)": 110,
            "График": "2/2",
            "Описание": "Смены по 11 часов",
        }
        rate = compute_hourly_rate(vac)
        self.assertAlmostEqual(rate, 666.7)

    def test_compute_hourly_rate_detects_time_range(self):
        vac = {
            "ЗП от (т.р.)": 120,
            "График": "5/2",
            "Описание": "Рабочий день с 9:00 до 18:00",
        }
        rate = compute_hourly_rate(vac)
        self.assertAlmostEqual(rate, 579.7)
...
class ExportShiftSheetTests(unittest.TestCase):
    def test_write_excel_creates_shift_sheet(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn(RATES_SHEET_BASE, xls.sheet_names)
            data = pd.read_excel(path, sheet_name=RATES_SHEET_BASE)
...
class SheetNameSanitizerTests(unittest.TestCase):
    def test_sanitize_replaces_forbidden_symbols(self):
        name = "Ставки (час/смена)"
        self.assertEqual(sanitize_sheet_name(name), "Ставки (час–смена)")
    ...
    def test_sanitize_drops_forbidden_chars(self):
        name = "Отчёт: 2025/11[черновик]"
    ...
    def test_unique_sheet_name_appends_suffix(self):
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / "wb.xlsx"
            df = pd.DataFrame({"A": [1]})
            with pd.ExcelWriter(path, engine="xlsxwriter") as writer:
                first = unique_sheet_name(writer, RATES_SHEET_BASE)
                df.to_excel(writer, sheet_name=first, index=False)
                second = unique_sheet_name(writer, RATES_SHEET_BASE)
